public class MyDataStructure {
    ...
    public void delete(int id) {
        ...
        toremove = OneTo5Tree.search(id);// check if the product is in the 1-5 tree.
        if (toremove != null) {
            productlist[toremove.satelliteData().quality()]--;
            OneTo5Tree.delete(toremove);
        }
        else {
            toremove = ZeroTree.search(id);// check if the product is in the 0 tree.
            if (toremove != null){
    ...
    private int countProducts() {
        // calculate the number of all the products.
        int productnum = 0;
        for (int i = 0; i < 6; i++) {
    ...
    public int qualitySum() {
        //calculate the sum of the qualitys of all products.
        int sum = 0;
        for (int i = 0; i < 6; i++) {
